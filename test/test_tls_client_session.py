...
class TestTlsClientSession(unittest.TestCase):
    def setUp(self):
        ...
        self.tls_client_session = TlsClientSession("example.com", self.on_connected, self.root_certificate_path, Mock())
    ...
    def test_should_return_client_hello_message_on_start(self):
        client_hello = self.tls_client_session.start()
    ...
    def test_should_call_transition_on_server_hello_message_received(self):
        with patch.object(TlsClientFsm, "transition") as mock_transition:
            session = TlsClientSession("example.com", Mock(), self.root_certificate_path, Mock())
    ...
    def test_should_compute_derived_secret_on_session_begin(self):
        self.tls_client_session.start()
        expected_derived_secret = bytes.fromhex("""6f 26 15 a1 08 c7 02 c5 67 8f 54 fc 9d ba
         b6 97 16 c0 76 18 9c 48 25 0c eb ea c3 57 6c 36 11 ba""")
        self.assertEqual(self.tls_client_session.derived_secret, expected_derived_secret)

    def test_should_compute_client_handshake_key_on_server_hello_received(self):
        self.tls_client_session.start()
        self.tls_client_session.on_record_received(self.server_hello)
        self.assertIs(len(self.tls_client_session.client_handshake_key), 16)
    ...
    def test_should_call_on_connected_on_handshake_finished_message_received(self):
        with patch.object(TlsClientSession, "server_handshake_key", new_callable=PropertyMock) as mock_handshake_key, \
            ...
            session = TlsClientSession("example.com", on_connected, self.root_certificate_path, Mock())
            session.start()
            session.on_record_received(self.server_hello)
            session.on_record_received(self.encrypted_extensions)
            session.on_record_received(self.server_certificate)
    ...
    def test_should_call_on_data_to_send_on_handshake_finished_message_received(self):
        with patch.object(TlsClientSession, "server_handshake_key", new_callable=PropertyMock) as mock_handshake_key, \
            ...
            session = TlsClientSession("example.com", Mock(), self.root_certificate_path, on_data_to_send)
            session.start()
            session.on_record_received(self.server_hello)
            session.on_record_received(self.encrypted_extensions)
            session.on_record_received(self.server_certificate)
